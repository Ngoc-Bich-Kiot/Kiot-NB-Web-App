...
import { FormProvider, RHFSelect, RHFTextField, RHFTextFieldNumber } from '@/components/hook_form';
...
export default function EditProduct() {
    ...
    const { handleSubmit, reset, watch, formState: { isSubmitting, isValid } } = methods;
    ...
    const onSubmit = async (data: EditProductFormInput) => {
        try {
            const currentImageUrls = data.productImages.filter((img): img is string => typeof img === "string");
            const imageIdsToDelete = initialImages
                .filter(img => !currentImageUrls.includes(img.urlPath))
                .map(img => img.id);

            const formData = convertToFormData({ ...data, ImageIdsToDelete: imageIdsToDelete });
            ...
            <Paper elevation={3} sx={{ p: 2 }}>
                ...
                <FormProvider methods={methods} onSubmit={handleSubmit(onSubmit)}>
                    <Grid container spacing={2}>
                        ...
                        <Grid item xs={12} sm={6}><RHFTextField name="originalPrice" label="Giá gốc" /></Grid>
                        <Grid item xs={12} sm={6}><RHFTextField name="sellingPrice" label="Giá bán" /></Grid>
                        <Grid item xs={12} sm={6}><RHFTextField name="importCosts" label="Giá nhập" /></Grid>
                        <Grid item xs={12} sm={6}><RHFTextField name="sourceOfProducts" label="Nguồn nhập" /></Grid>
                        <Grid item xs={12} sm={6}><RHFTextField name="stockQuantity" label="Số lượng tồn" /></Grid>
                        ...
                        <Grid item xs={12}><RHFMultiImageUpload name="productImages" label="Ảnh sản phẩm" /></Grid>
                    </Grid>
